...
export default function DiscoverCollection({
  ...
  useEffect(() => {
    async function fetchNewest() {
      try {
        const res = await fetch(`/api/products?per_page=${limit}&orderby=date&order=desc`);
  ...
  if (loading) {
    return (
      <section className="discover">
        ...
        <div className="dcv-viewport">
          <div className="dcv-track">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="dcv-slide dcv-slide--skeleton" />
            ))}
          </div>
        </div>
      </section>
        ...
        >
          {products.map((p, i) => {
            ...
            return (
              ...
              >
                <Link href={`/product?id=${p.id}`} className="dcv-card">
